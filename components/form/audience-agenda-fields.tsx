...
export default function EventAgenda() {
  ...
  // Watching for has_agenda state
  const has_agenda = useWatch({ control, name: "has_agenda" });
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm bg-card">
              <div className="space-y-1">
                <FormLabel className="font-semibold flex items-center gap-2">
                  <List className="w-5 h-5 text-primary" />
                  Add Agendas to the Event
                </FormLabel>
                <FormDescription className="text-sm text-muted-foreground">
                  Check to set Event Agendas
                </FormDescription>
              </div>
              <FormControl>
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    if (!checked) {
                      ...
                      if (fields.length === 0) append({ agenda_item: "" });
                    }
                  }}
                  className="data-[state=checked]:bg-primary"
                />
            ...
            <div className="space-y-3">
              {fields.map((field, index) => (
                ...
                >
                  ...
                  <FormField
                    control={control}
                    name={`agenda.${index}.agenda_item`} // ✅ Fixed field name
                    render={({ field }) => (
